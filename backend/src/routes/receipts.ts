...
import { uploadSingle, uploadReceiptImage } from '../middleware/upload'
import { ReceiptRequest, AuthenticatedRequest } from '../types'
...
router.get('/', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const { page = 1, limit = 50, sortBy = 'receiptDate', sortOrder = 'desc', year, month } = req.query as any
...
router.post('/', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      ...
      return
    }    const { storeName, receiptDate, totalAmount, eligibleAmount, imageUrl, imageFileName, imageMimeType, imageSize, items } = req.body as ReceiptRequest
...
router.put('/:id', authenticateToken, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const { id } = req.params
    const { storeName, receiptDate, totalAmount, eligibleAmount, imageUrl, imageFileName, imageMimeType, imageSize, items } = req.body as Partial<ReceiptRequest>
    ...
    if (receiptDate !== undefined) updateData.receiptDate = new Date(receiptDate)
    if (totalAmount !== undefined) updateData.totalAmount = totalAmount
    if (eligibleAmount !== undefined) updateData.eligibleAmount = eligibleAmount
    if (imageUrl !== undefined) updateData.imageUrl = imageUrl?.trim()
    if (imageFileName !== undefined) updateData.imageFileName = imageFileName?.trim()
    if (imageMimeType !== undefined) updateData.imageMimeType = imageMimeType?.trim()
...
router.post('/upload-image', authenticateToken, uploadReceiptImage, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({
        success: false,
...
router.post('/upload', authenticateToken, uploadSingle, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    if (!req.user) {
      res.status(401).json({
        success: false,
