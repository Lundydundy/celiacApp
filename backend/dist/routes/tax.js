...
router.get('/summary', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const medicalExpenses = await database_1.prisma.medicalExpense.findMany({
            where: {
                userId: req.user.id,
                date: {
                    gte: startDate,
        ...
        const totalMedicalExpenses = medicalExpenses.reduce((sum, expense) => sum + expense.amount, 0);
        ...
        for (let month = 0; month < 12; month++) {
            ...
            const monthReceipts = receipts.filter(r => r.receiptDate >= monthStart && r.receiptDate <= monthEnd);
            const monthMedical = medicalExpenses.filter((e) => e.date >= monthStart && e.date <= monthEnd);
            monthlyBreakdown.push({
                ...
                medical: {
                    ...
                    totalAmount: monthMedical.reduce((sum, e) => sum + e.amount, 0)
                }
            });
        } // Category breakdown for medical expenses
        const medicalByCategory = medicalExpenses.reduce((acc, expense) => {
...
router.get('/deduction-estimate', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
...
router.get('/profile/:year', auth_1.authenticateToken, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
        ...
        const taxProfile = await database_1.prisma.taxProfile.findUnique({
            where: {
                userId_year: {
                    userId: req.user.id,
                    year: year
...
router.put('/profile', auth_1.authenticateToken, async (req, res) => {
    try {
        ...
        const { year, netIncome, dependantIncome, claimingFor } = req.body;
        ...
        const taxProfile = await database_1.prisma.taxProfile.upsert({
            where: {
                userId_year: {
                    userId: req.user.id,
                    year: year
